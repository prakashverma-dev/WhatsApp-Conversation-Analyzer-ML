...
def sentiment_analysis(selected_user, df):
    ...
    sentiment_percent = round(
        (df['Sentiment'].value_counts(normalize=True) * 100), 2)

    # Average sentiment score
    avg_sentiment_score = round(df['Sentiment_Score'].mean(
    ), 3) if 'Sentiment_Score' in df.columns else None
...
def user_wise_sentiment(selected_user, df):
    ...
    user_sentiment = df.groupby(
        'user')['Sentiment'].value_counts().unstack(fill_value=0)
    user_sentiment['Total'] = user_sentiment.sum(axis=1)

    # Calculate percentage
    for sentiment in ['Positive', 'Negative', 'Neutral']:
        if sentiment in user_sentiment.columns:
            user_sentiment[f'{sentiment}_%'] = round(
                (user_sentiment[sentiment] / user_sentiment['Total']) * 100, 2)
